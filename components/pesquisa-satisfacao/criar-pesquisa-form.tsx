...
import { AlertCircle, Calendar, Trash2, Plus, MoveUp, MoveDown, AlertTriangle, Loader2 } from "lucide-react"
import { v4 as uuidv4 } from "uuid"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"

interface CriarPesquisaFormProps {
  pesquisaId?: string
  pesquisaData?: any
}

export function CriarPesquisaForm({ pesquisaId, pesquisaData }: CriarPesquisaFormProps) {
  ...
  const [tabPerguntasDisabled, setTabPerguntasDisabled] = useState(true)
  const tituloRef = useRef<HTMLInputElement>(null)
  const descricaoRef = useRef<HTMLTextAreaElement>(null)
  const dataInicioRef = useRef<HTMLInputElement>(null)
  const dataFimRef = useRef<HTMLInputElement>(null)

  const [pesquisa, setPesquisa] = useState<Partial<Pesquisa>>({
    titulo: "",
    descricao: "",
    dataInicio: "",
    dataFim: "",
  ...
  // Usar pesquisaData se fornecido
  useEffect(() => {
    if (pesquisaData) {
      console.log("Usando dados de pesquisa fornecidos:", pesquisaData)
      setPesquisa(pesquisaData)
      setIsEditMode(true)
      setTabPerguntasDisabled(false)
    }
  }, [pesquisaData])

  // Verificar se estamos em modo de edição ao carregar o componente
  useEffect(() => {
    if (pesquisaId && !pesquisaData) {
      setIsEditMode(true)
      carregarPesquisa(pesquisaId)
    }
  }, [pesquisaId, pesquisaData])

  // Carregar dados da pesquisa para edição
  const carregarPesquisa = async (id: string) => {
    try {
      ...
      const pesquisaCarregada = await PesquisaSatisfacaoService.obterPesquisaPorId(id)
      ...
      console.log("Pesquisa carregada:", pesquisaCarregada)

      // Preencher o formulário com os dados da pesquisa
      setPesquisa(pesquisaCarregada)
      setTabPerguntasDisabled(false)
  ...
  useEffect(() => {
    ...
    const datasValidas =
      !pesquisa.dataInicio || !pesquisa.dataFim || new Date(pesquisa.dataFim) > new Date(pesquisa.dataInicio)

    setTabPerguntasDisabled(!camposPreenchidos || !datasValidas)
  }, [pesquisa])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode && pesquisaId) {
        // Atualizar pesquisa existente
        await PesquisaSatisfacaoService.atualizarPesquisa(pesquisaId, pesquisa)
        toast({
        ...
        // Criar nova pesquisa
        await PesquisaSatisfacaoService.criarPesquisa(
          pesquisa as Omit<Pesquisa, "id" | "dataCriacao" | "qrCodeUrl" | "linkPublico">,
        )
        toast({
      ...
      console.error(`Erro ao ${isEditMode ? "atualizar" : "criar"} pesquisa:`, error)
      toast({
        title: "Erro",
        description: `Ocorreu um erro ao ${isEditMode ? "atualizar" : "criar"} a pesquisa`,
        variant: "destructive",
      })
    } finally {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <Button variant="outline" className="mt-4" onClick={() => router.push("/pesquisa-satisfacao/gerenciar")}>
          Cancelar
        </Button>
      </div>
    )
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-2">
          ...
          <TabsTrigger value="perguntas" disabled={tabPerguntasDisabled}>
            Perguntas
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="informacoes" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>{isEditMode ? "Editar Pesquisa" : "Informações da Pesquisa"}</CardTitle>
              <CardDescription>
                {isEditMode
                  ? "Edite as informações da sua pesquisa de satisfação"
                  : "Defina as informações básicas da sua pesquisa de satisfação"}
              </CardDescription>
            </CardHeader>
        ...
        <TabsContent value="perguntas" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>{isEditMode ? "Editar Perguntas" : "Perguntas da Pesquisa"}</CardTitle>
              <CardDescription>
                {isEditMode
                  ? "Edite as perguntas da sua pesquisa"
                  : "Adicione as perguntas que farão parte da sua pesquisa"}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
            ...
            <CardFooter className="flex justify-between">
              ...
              <Button type="submit" disabled={isSubmitting || !validarFormulario()}>
                {isSubmitting ? "Salvando..." : isEditMode ? "Atualizar Pesquisa" : "Salvar Pesquisa"}
              </Button>
            </CardFooter>
          </Card>
        </TabsContent>
