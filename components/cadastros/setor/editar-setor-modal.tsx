...
export function EditarSetorModal({ isOpen, onClose, onSave, setor }) {
  ...
  // Função para encontrar o valor do select com base no rótulo
  ...
  useEffect(() => {
    if (isOpen && setor) {
      // Encontrar os valores correspondentes nos selects
      const unidadeValue = findValueByLabel(unidadeOptions, setor.unidade)
      const turnoValue = findValueByLabel(turnoOptions, setor.turno)

      // Verificar se tem setor pai
      const hasSetorPai = !!setor.setorPai
      const setorPaiValue = hasSetorPai ? findValueByLabel(setorPaiOptions, setor.setorPai) : ""
      ...
      setFormData({
        ...
        turno: turnoValue,
        setorPai: hasSetorPai,
        setorPaiSelecionado: setorPaiValue,
      })
      setErrors({})
    }
  }, [isOpen, setor])
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md">
        <div className="bg-blue-500 text-white p-4 flex justify-between items-center rounded-t-lg">
          <h3 className="text-xl font-semibold text-gray-800">Editar Setor</h3>
        </div>
